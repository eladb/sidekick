...
sidekick_build_token() {
  ...
  local json
  json=$(cat <<EOF
{"v":1,"server_id":"${server_id}","base_url":"${base_url}","auth_token":"${auth_token}","cdp_url":"https://sidekick:${auth_token}@${host}","watch_url":"https://${host}/vnc.html?autoconnect=true&reconnect=true&token=${auth_token}","shell_url":"https://sidekick:${auth_token}@${host}/shell","exec_url":"https://sidekick:${auth_token}@${host}/exec","platform":"${platform}","created_at":"${created_at}"}
EOF
)
